    ...
    <div className="space-y-6">
        ...
        <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <input
              ...
              className="p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
            ...
            <input
              ...
              onChange={(e) => setNewCustomLink(prev => ({ ...prev, icon: e.target.value }))}
              className="p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
            ...
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-lg transition-all duration-200"
            >
              ➕ Add
            </button>
          </div>
      ...
      >
        ...
        <div className="flex flex-wrap gap-3">
          {socialPlatforms
            .filter(platform => socialLinks[platform.key] && validateUrl(socialLinks[platform.key], platform.key))
            .map(platform => (
              <a
                key={platform.key}
                href={getDisplayUrl(socialLinks[platform.key], platform.key)}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center space-x-2 bg-white dark:bg-gray-800 px-3 py-2 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 text-gray-700 dark:text-gray-300 hover:text-purple-600 dark:hover:text-purple-400"
              >
                <span>{platform.icon}</span>
        ...
        {Object.values(socialLinks).filter(Boolean).length === 0 && customLinks.length === 0 && (
          <p className="text-center text-gray-500 dark:text-gray-400">
            Add social links above to see the preview
          </p>
        )}
      </motion.div>
    </div>
